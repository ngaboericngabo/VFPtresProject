    ...
    public AbstractDao() {
     ...
     public  void configConnection(){
      ...
      try {
            ...
            factory = new Configuration().configure("hibernate.cfg.xml").addAnnotatedClass(entityClass).buildSessionFactory();
...
int q = (Integer)session.createQuery(AnySQLQuerry).uniqueResult();
  ...
  /*  protected Session getMyCurrentSession() {
    ...
    public List getGenericListWithHQLParameter(final String[] propertyName,
            final Object[] value, final String hqlStatement, final String orderSatment)
            throws Exception {
       configConnection();
        Transaction t = session.beginTransaction();
        if (entityClass == null) {
    ...
    public List getGenericListWithHQLParameterEndByEnd(final String[] propertyName,
            final Object[] value, final String hqlStatement, final String[] propertyName2,
            final Object[] value2,final String orderSatment)
            throws Exception {
        configConnection();
        Transaction t = session.beginTransaction();
        if (entityClass == null) {
...
int i=0;
    ...
    public List getGenericListWithHQLParameteroOfOrCondition(final String[] propertyName,
            final Object[] value, final String hqlStatement, final String orderSatment)
            throws Exception {
       configConnection();
        Transaction t = session.beginTransaction();
